        ...
        if (pCur != null) {
            if (pCur.getCount() > 0) {
                ...
                while (pCur.moveToNext()) {
                    Integer contactId = pCur.getInt(pCur.getColumnIndexOrThrow(PHONE_CONTACT_ID));
                ...
                if (cur != null) {
                    if (cur.getCount() > 0) {
                        ...
                        while (cur.moveToNext()) {
                            int id = cur.getInt(cur.getColumnIndexOrThrow(CONTACT_ID));
                            if (phones.containsKey(id)) {
                                Contact con = new Contact();
                                con.setMyId(id);
                                con.setName(cur.getString(cur.getColumnIndexOrThrow(DISPLAY_NAME)));
                                con.setPhone(TextUtils.join(",", Objects.requireNonNull(phones.get(id)).toArray()));
                                contacts.add(con);
                            }
                        }
                        return contacts;
